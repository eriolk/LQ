...
The amplitudes corresponding to the neutral diagrams depicted in Fig.~ are given by
\bea
\label{eq112}
\mathcal{M}_1 & = & -\lambda^{i*}_{\ell}\lambda^j_{\ell^{\prime}}\mathbb{P}_{\chi_1}(p+k)[\,\xbar{v}_R(\nu_{\ell};p)\,u_L(U_i;k)\,][\,\xbar{u}_L(U_j;r)\,v_R(\nu_{\ell^{\prime}};q)\,] \\
\label{eq113}
\mathcal{M}_2 & = & -\frac{\lambda^{i*}_{\ell}\lambda^j_{\ell^{\prime}}\mathbb{P}_{\chi_2}(p+k)}{2}[\,\xbar{v}_R(\nu_{\ell};p)\,u_L(D_i;k)\,][\,\xbar{u}_L(D_j;r)\,v_R(\nu_{\ell^{\prime}};q)\,] \\
\label{eq114}
\mathcal{M}_3 & = & -\lambda^{j*}_{\ell}\lambda^i_{\ell^{\prime}}\mathbb{P}_{\chi_1}(p-r)[\,\xbar{v}_R(\nu_{\ell};p)\,v_L(\xbar{U}_j;r)\,][\,\xbar{v}_L(\xbar{U}_i;k)\,v_R(\nu_{\ell^{\prime}};q)\,] \\
\label{eq115}
\mathcal{M}_4 & = & -\frac{\lambda^{j*}_{\ell}\lambda^i_{\ell^{\prime}}\mathbb{P}_{\chi_2}(p-r)}{2}[\,\xbar{v}_R(\nu_{\ell};p)\,v_L(\xbar{D}_j;r)\,][\,\xbar{v}_L(\xbar{D}_i;k)\,v_R(\nu_{\ell^{\prime}};q)\,],
\eea
...
\label{eq117}
|\mathcal{M}_2|^2_{\mathrm{unpol.}} & = & |\lambda^{i}_{\ell}\lambda^j_{\ell^{\prime}}|^2|\mathbb{P}_{\chi_2}(p+k)|^2 \frac{\,\hat{s}^2}{8\,} \\
\label{eq118}
|\mathcal{M}_3|^2_{\mathrm{unpol.}} & = & |\lambda^{j}_{\ell}\lambda^i_{\ell^{\prime}}|^2|\mathbb{P}_{\chi_1}(p-r)|^2\frac{\hat{u}(\hat{u}-m^2_{\scriptsize\xbar{U}_j})}{2}\ \\
\label{eq119}
|\mathcal{M}_4|^2_{\mathrm{unpol.}} & = & |\lambda^{j}_{\ell}\lambda^i_{\ell^{\prime}}|^2|\mathbb{P}_{\chi_2}(p-r)|^2\frac{\hat{u}^2}{8},
\eea
where $\mathbb{P}_{x}(p)\equiv (p^2-m^2_x-im_x\Gamma_x)^{-1}$. For the charged cases shown in Fig.~ we have
\bea
\label{eq120}
\mathcal{M}_{I} & = & -\frac{\lambda^{i*}_{\ell}\lambda^j_{\ell^{\prime}}\mathbb{P}_{\chi_2}(p+k)}{2}[\,\xbar{v}_R(\nu_{\ell};p)\,u_L(D_i;k)\,][\,\xbar{u}_L(U_j;r)\,v_R(\nu_{\ell^{\prime}};q)\,] \\
\label{eq121}
\mathcal{M}_{II} & = & -\frac{\lambda^{j*}_{\ell}\lambda^i_{\ell^{\prime}}\mathbb{P}_{\chi_2}(p-r)}{2}[\,\xbar{v}_R(\nu_{\ell};p)\,v_L(\xbar{D}_j;r)\,][\,\xbar{v}_L(\xbar{U}_i;k)\,v_R(\nu_{\ell^{\prime}};q)\,],
\eea
...
\eeq
Under the assumption that the interaction of the LQs with fermions proceeds only between the first family of quarks and the first and second families of leptons, the above expressions can be simplified as follows,
\bea
\label{eq127}
